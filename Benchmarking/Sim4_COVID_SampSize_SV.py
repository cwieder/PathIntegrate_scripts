...
for sampsize in sizes:
    for effect in effects:
        ...
        for train_index, test_index in skf.split(metabsim.iloc[:, :-1], metabsim_orig['Group']):
            ...
            concat_omics_train = pd.concat([X_train_m, X_train_p], axis=1)
            concat_omics_test = pd.concat([X_test_m, X_test_p], axis=1)
            ...
            metrics['AUC'].append(auc_sv)

metrics_df = pd.DataFrame(metrics)
# save results to csv
metrics_df.to_csv('COVID/SampSize/COVID_SampSize_AUC_SV_'+pathway_enriched+'.csv')
