...
prot_kegg = pd.read_csv('../COVID/prot_kegg.csv', index_col=0)
metab_kegg = pd.read_csv('../COVID/metab_kegg.csv', index_col=0)
# impute missing values with median
prot_kegg_imp = prot_kegg.iloc[:, :-1].fillna(prot_kegg.iloc[:, :-1].median())
metab_kegg_imp = metab_kegg.iloc[:, :-1].fillna(metab_kegg.iloc[:, :-1].median())
prot_kegg_imp['Group'] = [1]*63 + [0]*64
metab_kegg_imp['Group'] = [1]*63 + [0]*64
...
mo_paths = mo_paths.sample(215, random_state=1, replace=False)
mo_paths_moleculesize = mo_paths_moleculesize.sample(215, random_state=1, replace=False)
...
dsets = [[metab_imp, prot_imp], [metab_kegg_imp, prot_kegg_imp], [metab_imp, prot_imp]]
...
res_all.to_csv('COVID/Comparison/COVID_comparison_AUC_'+str(pathway_idx)+'.csv')
