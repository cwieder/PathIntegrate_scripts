...
def id_target_pathway(dsets, effect, enrich_paths, paths_dict):
    ...
    with (ro.default_converter + pandas2ri.converter).context():
        ...
        r_design_mat = ro.conversion.get_conversion().py2rpy(design_mat)
        r_labels_kpca = ro.IntVector(kpca_scores_prot['Group'].tolist())
    ...
    test_keepX = ro.ListVector([('Metabolomics_kpca', ro.IntVector([150, 100, 50, 25, 5]) ), ('Proteomics_kpca', ro.IntVector([200, 100, 50, 25, 5]))])
    ...
    for i in range(0, 25):
        ...
        # balanced classes to avoid errror
        met_sample = metab[metab['Group'] == 0].sample(n=200, replace=True).append(metab[metab['Group'] == 1].sample(n=200, replace=True))
        prot_sample = prot.loc[met_sample.index, :]
...
# for e in [0]:
for e in [0, 0.1, 0.25, 0.5, 0.75, 1]:

    r_m1, r_sv, diablo_loadings, diablo_sparse = id_target_pathway([metab, prot], e, [pathway_enriched], mo_paths_dict)
...
effects_df_mv.to_csv('COPD/Target/target_MBPLS_Reactome_GOLD_'+pathway_enriched+'.csv')
effects_df_sv.to_csv('COPD/Target/target_SV_Reactome_GOLD_'+pathway_enriched+'.csv')
effects_df_diablo_loading.to_csv('COPD/Target/target_DIABLO_loadings_Reactome_GOLD_'+pathway_enriched+'.csv')
effects_df_diablo_sparse.to_csv('COPD/Target/target_DIABLO_sparse_Reactome_GOLD_'+pathway_enriched+'.csv')